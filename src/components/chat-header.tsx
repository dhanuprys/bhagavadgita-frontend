...
export function ChatHeader({ messageCount, isMobile }: ChatHeaderProps) {
    ...
    return (
      <div className="flex items-center gap-3">
        <motion.div
          ...
          className="w-10 h-10 rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center shadow-sm border border-slate-200/50"
        >
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <span className="font-semibold text-slate-700">Chat dengan BhagavadAI</span>
        {messageCount > 0 && <span className="text-xs text-slate-500 ml-2">({messageCount} chat)</span>}
      </div>
    </div>
  )
}
