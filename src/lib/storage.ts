        ...
        try {
            ...
            if (error instanceof Error && error.name === 'QuotaExceededError') {
                ...
                try {
                    ...
                    console.error(
                        'Failed to save conversation after cleanup:',
                        retryError
                    );
                }
            }
...
export function pickCrossPagePrompt(): string | null {
    const prompt = localStorage.getItem('cross-page-prompt');

    if (prompt) {
        localStorage.removeItem('cross-page-prompt');
        return prompt;
    }
